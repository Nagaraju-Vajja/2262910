...
@Component
public class BOPSOrderValidations extends OrderValidations  {

    public BOPSOrderValidations(ICollectorderResponsePublishService collectorderResponsePublishService, IEventLogService eventLogService) {
    ...
    public void validate(Order order) throws ExecutionException, InterruptedException {
        if (null != order && !CollectionUtils.isEmpty(order.getOrderLines())) {
            for (OrderLine ol : order.getOrderLines()) {
                if (StringUtils.isNotEmpty(ol.getFulfillmentType()) && BOPS.equalsIgnoreCase(ol.getFulfillmentType())) {
                    if (!CollectionUtils.isEmpty(ol.getOrderLineStatuses())) {
                        for (OrderLineStatus ols : ol.getOrderLineStatuses()) {
                            if (StringUtils.isEmpty(ols.getPickupDivision())) {
                                publishAndThrowError(OrderValidationErrorCodes.ORDER_LINE_STATUS_PICKUPDIVISION_MISSING_BOPS, order);
                            }
                            if (StringUtils.isEmpty(ols.getPickupLocation())) {
                                publishAndThrowError(OrderValidationErrorCodes.ORDER_LINE_STATUS_PICKUPLOCATION_MISSING_BOPS, order);
                            }
                            if (StringUtils.isEmpty(ols.getPickupStore())) {
                                publishAndThrowError(OrderValidationErrorCodes.ORDER_LINE_STATUS_PICKUPSTORE_MISSING_BOPS, order);
                            }
                            if (StringUtils.isEmpty(ols.getPickupChannel())) {
                                publishAndThrowError(OrderValidationErrorCodes.PICKUP_CHANNEL_MISSING_BOPS, order);
                            }
                        ...
                        if (null == ol.getAddressDetails()) {
                            publishAndThrowError(OrderValidationErrorCodes.ADDRESS_DETAILS_ERROR_BOPS, order);
                        }
                        if (!CollectionUtils.isEmpty(ol.getAddressDetails())) {
                            for (Address address : ol.getAddressDetails()) {
                                if (StringUtils.isEmpty(address.getType())) {
                                    publishAndThrowError(OrderValidationErrorCodes.ADDRESSDETAILS_MISSING_FIELDS_ERROR_BOPS, order);
                                }
                            }
                        }
                    }
