...
@Component
@Slf4j
public class BLCOMDigitalCheckOutStream extends AbstractStreamSubscriber {
    @Value("${topic.subscription.digitalcheckout.blcom.name}")
    ...
    public BLCOMDigitalCheckOutStream(PubSubTemplate pubSubTemplate, Tracer tracer,
                                      OrderErrorMessagePublisher orderErrorMessagePublisher,
    ...
    protected void invokeService(PubsubMessage message) {
        if (messageValidator.validateMessage(message)) {
            try {
                ...
                orderSource.setSourceSystem(BCHECKOUT_SOURCE_SYSTEM);
                ordercollectorchestratorService.collectOrder(orderSource);
            } catch (Exception e) {
                if(e instanceof JsonProcessingException){
                    log.error("Exception while parsing BCOM order json {} messageId -> {} input payload -> {}",e.getMessage(),message.getMessageId(),message.getData().toStringUtf8());
    ...
    private void publishAckIfError(Exception e,PubsubMessage message) throws ExecutionException, InterruptedException {
        ...
        order.setSourceSystem(BCHECKOUT_SOURCE_SYSTEM);
